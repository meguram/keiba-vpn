...
ワーカー（API のキュー処理、または ``python -m src.scraper.queue_worker``）が処理すると、
GCS の ``horse_training`` とローカル ``data/local/horse_training/{shard4}/`` に保存される。

例::

    python3 -m src.research.scripts.enqueue_horse_training_all
    python3 -m src.research.scripts.enqueue_horse_training_all --dry-run
    # 全キューを空にしてから全馬再投入（precheck 即時 → 未所持のみ待機、ワーカキック）
    python3 -m src.research.scripts.enqueue_horse_training_all --replace-all
    # 全消去なしで投入だけしつつ、手元でワーカを起動したい場合
    python3 -m src.research.scripts.enqueue_horse_training_all --kick
"""
...
from src.pipeline.build_horse_entity_store import collect_horse_ids_from_tables  # noqa: E402
from src.pipeline.features.feature_store import TABLES_DIR  # noqa: E402
from src.scraper.job_queue import ScrapeJobQueue, kick_process_queue_background  # noqa: E402
