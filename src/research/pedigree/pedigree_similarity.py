...
Usage:
  python -m src.research.pedigree.pedigree_similarity [--min-progeny 20]
  python -m src.research.pedigree.pedigree_similarity --mode dual --lambda-paternal 0.35 --method overlap

GCS 負荷:
  ``load_pedigrees_from_storage`` はまず ``research.pedigree.pedigree_local_store`` で
  スナップショット ``horse_pedigree_5gen.jsonl.gz`` をローカル化し、
  **list_keys + 全頭 load を避ける**。未取得時のみ従来の全件走査にフォールバック。
  オフライン整備: ``python -m src.research.pedigree.sire_factor_stats`` の snapshot 生成、または
...
logger = logging.getLogger("research.pedigree.pedigree_similarity")
...
def parse_blood_table_5gen(html: str) -> list[dict[str, Any]]:
    ...
    for td in bt.select("td"):
        ...
        # 空白スロット（リンクなし）でもポジションカウンタは必ず進める。
        # 進めないと後続の祖先のポジションがずれる。
...
def load_pedigrees_from_storage(
    ...
    戦略:
      1. **推奨**: GCS スナップショット ``horse_pedigree_5gen.jsonl.gz`` を1回取得し
         ``research.pedigree.pedigree_local_store`` 経由でローカルインデース化（list_keys しない）。
...
def main():
    ...
    _r = str(Path(__file__).resolve().parents[3])
