...
Usage:
  python3 -m src.research.race.evaluate_track_bias_pedigree_only
  python3 -m src.research.race.evaluate_track_bias_pedigree_only --parquet data/modeling/layer_a_train.parquet
"""
    ...
    *,
    ...
    if missing:
        raise ValueError(
            ...
            "`python3 -m src.pipeline.build_layer_a_dataset`（track_bias 付き）を実行してください。"
            f" 不足: {missing}"
        )
    ...
    if tr.sum() == 0 or va.sum() == 0 or te.sum() == 0:
        raise RuntimeError(
            ...
            "Parquet にマニフェスト対象年が含まれるよう `python3 -m src.pipeline.build_layer_a_dataset` で全期間ビルドするか、"
