...
Usage:
  python -m src.scraper.run race-result 202505020811
  python -m src.scraper.run race-card 202606020611
  python -m src.scraper.run horse 2021105354
  python -m src.scraper.run horse-training 2021105143   # 調教タイム（最大20ページ、要ログイン）
  python -m src.scraper.run horse-training 2021105143 --force  # GCS+ローカル済でも再取得
  python -m src.scraper.run race-list 20260322
  python -m src.scraper.run race-detail 202606020611
...
from src.scraper.horse_training_local import (
...
class ScraperRunner:
    ...
    def _fetch_parse_save(
        ...
        try:
            ...
            if category == "race_shutuba" and isinstance(data, dict):
                try:
                    from src.pipeline.sync_ped_tbl_for_horses import sync_ped_tbl_after_shutuba
    ...
    def scrape_smartrc(self, race_id: str, date: str = "") -> dict | None:
        ...
        from src.scraper.smartrc_client import netkeiba_to_rcode
    ...
    def _init_smartrc(self):
        ...
        try:
            from src.scraper.smartrc_client import SmartRCClient
