...
馬ごとに連続レースを追い、条件（距離バンド × 馬場種別）が変化した際の
megu_index 変化量（delta = 転換後 - 転換前）を集計して DB に投入する。
...
大きな条件変更:
  - 馬場種別変更: 芝 ↔ ダート（どの距離バンド間でも記録）
  - 距離バンド変更: 隣接バンドを跨ぐものも全て記録（バンド内移動は除外）

Usage:
    python3 -m src.scripts.data.compute_condition_transfer [--model-version stg-v1] [--min-samples 5]
"""
...
MIN_SAMPLES = 5  # 転換係数として採用する最低サンプル数
...
def fetch_megu_history(engine) -> pd.DataFrame:
    """馬ごとの megu_index 全履歴（レース日付順）を取得。"""
    sql = """
        SELECT
            mi.horse_id,
            mi.race_id,
...
def compute_transfer_deltas(df: pd.DataFrame) -> pd.DataFrame:
    """
    馬ごとに連続レース対を走査し、条件が変化した対の delta を収集する。
    同一 race_date の複数レースは直前の異なる日付のレースと対比する。
    """
    records = []
    for horse_id, grp in df.groupby("horse_id", sort=False):
        ...
        for i in range(1, len(races)):
            ...
            surface_changed = sf != st
            band_changed = db_from != db_to

            if not surface_changed and not band_changed:
                continue  # 同条件は記録しない

            delta = float(curr["megu_index"]) - float(prev["megu_index"])
            records.append({
                "surface_from": sf,
                "surface_to": st,
...
def aggregate_transfer_coeffs(deltas: pd.DataFrame, min_samples: int) -> pd.DataFrame:
    """転換ペアごとに delta の平均・標準偏差・サンプル数を集計。"""
    if deltas.empty:
        return pd.DataFrame()

    group_cols = ["surface_from", "surface_to", "dist_band_from", "dist_band_to"]
    agg = (
        deltas.groupby(group_cols)["delta"]
        .agg(
            delta_mean="mean",
            delta_std="std",
            sample_count="count",
    ...
    agg["delta_mean"] = agg["delta_mean"].round(2)
    agg["delta_std"] = agg["delta_std"].round(2)

    before = len(agg)
    agg = agg[agg["sample_count"] >= min_samples].copy()
    logger.info("転換係数: %d ペア（最低 %d サンプル未満 %d ペア除外）",
                len(agg), min_samples, before - len(agg))

    for _, row in agg.iterrows():
        logger.info(
            "  %s %s → %s %s : delta=%.2f ± %.2f (n=%d)",
...
def upsert_transfer_coeffs(engine, agg: pd.DataFrame, model_version: str) -> int:
    """megu_condition_transfer テーブルに upsert。"""
    from src.db.models import MeguConditionTransfer
...
def main():
    parser = argparse.ArgumentParser(description="条件転換係数計算・DB投入")
    ...
    logger.info("=== Step 2: 条件転換 delta 計算 ===")
    deltas = compute_transfer_deltas(df)
    if deltas.empty:
        logger.warning("条件転換対が0件でした。データが1レースのみの馬しかいない可能性があります。")
        sys.exit(0)
