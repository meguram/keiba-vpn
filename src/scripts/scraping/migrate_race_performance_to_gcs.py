...
使い方:
  ...
  python3 -m src.scripts.scraping.migrate_race_performance_to_gcs --dry-run

  # 実行（アップロード後にローカルファイルを削除）
  python3 -m src.scripts.scraping.migrate_race_performance_to_gcs --delete-after-upload

  # アップロードのみ（削除はしない）
  python3 -m src.scripts.scraping.migrate_race_performance_to_gcs
"""
