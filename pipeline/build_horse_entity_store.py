...
- ``ped_tbl``: ``data/local/horse_pedigree_5gen/{horse_id 先頭4文字}/{horse_id}.json`` があればロング表化。
  ...
  **クロス列**（父系/母系/全体の重複・濃度・両系統出現・主馬単位集計）は ``pipeline.horse_pedigree_cross`` を参照。
...
- ``training_tbl``: ``--training-json-dir`` を指定し ``{horse_id}.json`` が存在する馬のみ書き出し
  ...
  python -m pipeline.build_horse_entity_store --overwrite
  python -m pipeline.build_horse_entity_store --max-horses 5000 --overwrite
  python -m pipeline.build_horse_entity_store --overwrite --merge-gen5-sires
...
from pipeline.feature_store import TABLES_DIR
from pipeline.horse_entity_layout import (
    HORSE_ENTITY_ROOT,
    PED_TBL,
    RESULT_TBL,
    TRAINING_TBL,
...
from pipeline.horse_pedigree_cross import PED_CROSS_COLUMN_NAMES, add_pedigree_cross_columns
