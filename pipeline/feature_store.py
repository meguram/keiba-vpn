...
from pipeline.feature_layout import (
    BLOCK_BASE_TBL,
...
class FeatureStore:
    ...
    def _merge_keys_for_column(self, name: str) -> list[str]:
        ...
        if BLOCK_BASE_TBL in parts:
            from pipeline.feature_layout import MERGE_KEYS_BASE_TBL

            return list(MERGE_KEYS_BASE_TBL)
        if BLOCK_RACE_TBL in parts:
            from pipeline.feature_layout import MERGE_KEYS_RACE_TBL

            return list(MERGE_KEYS_RACE_TBL)
        if "race_jockey_tbl" in parts:
            from pipeline.feature_layout import MERGE_KEYS_RACE_JOCKEY_TBL

            return list(MERGE_KEYS_RACE_JOCKEY_TBL)
        if "race_trainer_tbl" in parts:
            from pipeline.feature_layout import MERGE_KEYS_RACE_TRAINER_TBL
        ...
        if "jockey_tbl" in parts:
            from pipeline.feature_layout import MERGE_KEYS_JOCKEY_TBL

            return list(MERGE_KEYS_JOCKEY_TBL)
        if "trainer_tbl" in parts:
            from pipeline.feature_layout import MERGE_KEYS_TRAINER_TBL

            return list(MERGE_KEYS_TRAINER_TBL)
        if "horse_tbl" in parts:
            from pipeline.feature_layout import MERGE_KEYS_HORSE_TBL
