...
列: ``race_id``, ``horse_id``, ``rank``（ソースの ``finish_position`` を数値化。失敗は null）。
  ...
  python -m pipeline.build_rank_target
  python -m pipeline.build_rank_target --years 2024 2025 --overwrite
"""
...
from pipeline.feature_layout import FEATURES_DIR, RANK_TBL_SUBDIR
