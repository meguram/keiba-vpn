...
騎手・調教師の成績統計 Parquet を生成する。
  ...
  python -m pipeline.build_jockey_trainer_stats
  python -m pipeline.build_jockey_trainer_stats --years 2022,2023,2024

既定は JRA（venue_code 01–10）のみ。地方を含める場合は ``--nar``。

定期実行はリポジトリの ``scripts/update_jockey_trainer_stats.sh`` と
``scripts/setup_jockey_trainer_stats_cron.sh``（``install``）を利用。
"""
...
from pipeline.jockey_trainer_stats import write_jockey_trainer_stats
