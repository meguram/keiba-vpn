...
``race_horse_tbl``（キー: race_id + horse_id）へ移し、レジストリを更新する。
  ...
  python -m pipeline.migrate_legacy_feature_columns
  python -m pipeline.migrate_legacy_feature_columns --dry-run
...
from pipeline.feature_layout import BLOCK_RACE_HORSE_TBL, MERGE_KEYS_RACE_HORSE_TBL
from pipeline.feature_store import FeatureStore
from pipeline.id_value_policy import sanitize_netkeiba_string_id
from pipeline.race_performance import FEATURE_COLUMNS
