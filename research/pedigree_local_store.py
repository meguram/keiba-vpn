...
def stallion_pedigree_closure_horse_ids(
    ...
    *,
    ...
    1 頭の完全な 5 世代表には牡が ``research.pedigree_similarity.MALE_PEDIGREE_SLOTS_5GEN`` (=31)
    スロットあり（種牡馬因子の 6 スロット加権とは別。こちらは表の牡をすべて列挙）。

    インデックスに無い馬は集合に含めるが、そこからは展開しない（未取得はワーカー取得後に
    再投入バッチで辿れる）。
    """
    from research.pedigree_similarity import male_pedigree_slot_horse_ids_from_ancestors
