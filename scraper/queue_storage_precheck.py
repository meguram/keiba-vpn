...
from scraper.horse_training_local import horse_training_json_path
from scraper.run import _horse_training_local_dict_looks_stored, _read_horse_training_local_file
...
def _tasks_normalized_for_job(j: dict) -> list[str]:
    from scraper.queue_tasks import normalize_tasks
...
def _jra_race_ids_on_date_cached(
    ...
    if dk in cache:
        return cache[dk]
    from scraper.verify_scrape_completeness import _jra_race_ids_on_date
...
def collect_gcs_key_requirements(
    ...
    """
    from scraper.verify_horse_scrape_completeness import _horse_task_storage_category
