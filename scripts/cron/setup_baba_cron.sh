...
#   bash scripts/cron/setup_baba_cron.sh --status     # 状態確認
#   bash scripts/cron/setup_baba_cron.sh --schedule   # 直近のスケジュール表示
...
show_schedule() {
    cd "${SCRIPT_DIR}"
    ${PYTHON} -m src.scraper.jra_baba_live --schedule --schedule-days "${2:-30}"
}
