...
- `RACE_STORE_SAMPLE_*` — `race_performance/races/` の JSON 抜粋
  ...
  python scripts/embed_raw_flat_samples_in_reference_html.py
"""
...
from pipeline.raw_table_features import (  # noqa: E402
    INDEX_SPECS,
...
def build_keys_sample_fragment() -> str:
    ...
    if df.empty:
        return (
            ...
            '<span class="mono">python -m pipeline.register_raw_table_features</span> で生成してください。）</p>'
        )
...
def build_fragment() -> str:
    ...
    lines = [
        ...
        '    <p class="small">登録は <span class="mono">python -m pipeline.register_raw_table_features</span>。'
    ...
    if jt_path is None or not jt_path.is_file():
        return "\n".join(
            [
                ...
                " <span class=\"mono\">python -m pipeline.build_jockey_trainer_stats</span> で生成してください。</p>",
                "      </div>",
                "    </details>",
            ]
        )
