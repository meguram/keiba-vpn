...
#
# Usage:
#   bash scripts/setup_jockey_trainer_stats_cron.sh           # 表示のみ
#   bash scripts/setup_jockey_trainer_stats_cron.sh install   # crontab に追加
#   bash scripts/setup_jockey_trainer_stats_cron.sh remove    # 削除
#   bash scripts/setup_jockey_trainer_stats_cron.sh status    # 直近ログ・manifest
# =============================================================================

set -euo pipefail

PROJECT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
UPDATE_SH="${PROJECT_DIR}/scripts/update_jockey_trainer_stats.sh"
...
#   pipeline.build_jockey_trainer_stats → data/features/jockey_trainer_stats/
