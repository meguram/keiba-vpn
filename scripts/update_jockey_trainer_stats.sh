...
#
# Usage:
#   bash scripts/update_jockey_trainer_stats.sh
#   bash scripts/update_jockey_trainer_stats.sh --nar   # 地方開催も含める
#
# cron 例は scripts/setup_jockey_trainer_stats_cron.sh を参照。
# =============================================================================
...

{
  ...
  if ! "$PYTHON" -m pipeline.build_jockey_trainer_stats "${EXTRA[@]}"; then
    ec=$?
