"""
ARCHITECTURE.md 仕様書 自動生成ユーティリティ

Orchestrator の実際のインスタンスをイントロスペクトし、
全エージェントの責務・入出力・通信フロー・API仕様を
網羅した技術仕様書を生成する。

README.md と同時に自動更新される。
"""
...
def generate_spec(orch: Orchestrator, base_dir: str = ".") -> str:
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    sections = [
        _header(now),
        _design_philosophy(),
        _hierarchy_diagram(orch),
        _agent_catalog(orch),
        _intent_routing(),
        _pipeline_flow(),
        _communication_patterns(),
        _data_architecture(base_dir),
        _mlflow_integration(),
        _mlflow_ui(),
        _web_dashboard(),
        _prediction_output_format(),
        _api_reference(),
...
def _header(now: str) -> str:
    return f"""# ML-AutoPilot Keiba — システム仕様書
...
def _hierarchy_diagram(orch: Orchestrator) -> str:
    lines = ["", "## 2. 階層構成", "", "```"]
    lines.append("将軍 (Orchestrator)")
    lines.append("│")
    lines.append("├── 意図分類エンジン (parse_command)")
    lines.append("│     スコアリング方式で 6カテゴリ × 20+ アクションに分類")
    lines.append("│")
    ...
    for i, (mgr, desc, duties) in enumerate(managers_info):
        is_last = i == len(managers_info) - 1
        connector = "└──" if is_last else "├──"
        child = "    " if is_last else "│   "

        lines.append(f"{connector} {mgr.role} ({mgr.name})")
        lines.append(f"{child} 責務: {desc}")

        sub_list = list(mgr.sub_agents.items())
        if sub_list:
            lines.append(f"{child} 配下:")
            for j, (key, agent) in enumerate(sub_list):
                sub_last = j == len(sub_list) - 1
                sub_conn = "└──" if sub_last else "├──"
                lines.append(f"{child}   {sub_conn} {key} ({agent.role})")
        for d in duties:
            lines.append(f"{child} • {d}")
        if not is_last:
            lines.append("│")
...
def _agent_catalog(orch: Orchestrator) -> str:
    lines = [
        "", "## 3. エージェントカタログ", "",
    ]

    # 将軍
    lines.append("### 3.0 将軍 (Orchestrator)")
    lines.append("")
    lines.append("| 属性 | 値 |")
    lines.append("|------|-----|")
    lines.append("| クラス | `Orchestrator` |")
    lines.append("| ファイル | `agents/orchestrator.py` |")
    lines.append("| 責務 | 意図分類、家老への委譲、結果返却 |")
    lines.append(f"| 現在状態 | {orch.status.value} |")
    lines.append("")
    ...
    manager_specs = {
        "ConfigMgr": {
            "class": "ConfigManager",
            "file": "agents/config_manager.py",
            "desc": "設定・要件管理を統率する家老",
            "inputs": "ユーザーの設定変更コマンド、自然言語要件",
            "outputs": "MLProjectSpec、設定レポート",
            "api": "`generate_spec()`, `handle_show()`, `handle_change()`, `handle_reset()`, `add_requirements()`",
        },
        "PipelineMgr": {
            "class": "PipelineManager",
            "file": "agents/pipeline_manager.py",
            "desc": "ML予測パイプラインの全6フェーズを統率する家老",
            "inputs": "MLProjectSpec (軍師経由)",
            "outputs": "PredictionResult、MLflow run_id",
            "api": "`run_pipeline(spec)` → `(PredictionResult, elapsed)`",
        },
        "DataMgr": {
            "class": "DataManager",
            "file": "agents/data_manager.py",
            "desc": "スクレイピング・データ確認・エクスポートを統率する家老",
            "inputs": "スクレイピングコマンド (action, params)",
            "outputs": "スクレイピング結果、データ概要レポート",
            "api": "`handle_scrape()`, `handle_preview()`, `handle_stats()`, `handle_columns()`, `handle_export()`",
        },
        "InfoMgr": {
            "class": "InfoManager",
            "file": "agents/info_manager.py",
            "desc": "情報照会・評価報告を統率する家老",
            "inputs": "他3家老への参照 (`set_peers()`)",
            "outputs": "状態レポート、評価レポート、概要レポート",
            "api": "`handle_status()`, `handle_evaluate()`, `handle_importance()`, `handle_summary()`",
        },
    }

    managers = [orch._config_mgr, orch._pipeline_mgr, orch._data_mgr, orch._info_mgr]
    for idx, mgr in enumerate(managers, 1):
        spec = manager_specs.get(mgr.name, {})
        lines.append(f"### 3.{idx} {mgr.role} ({mgr.name})")
        lines.append("")
        lines.append("| 属性 | 値 |")
        lines.append("|------|-----|")
        lines.append(f"| クラス | `{spec.get('class', mgr.name)}` |")
        lines.append(f"| ファイル | `{spec.get('file', '')}` |")
        lines.append(f"| 責務 | {spec.get('desc', '')} |")
        lines.append(f"| 入力 | {spec.get('inputs', '')} |")
        lines.append(f"| 出力 | {spec.get('outputs', '')} |")
        lines.append(f"| 公開API | {spec.get('api', '')} |")
        lines.append(f"| 現在状態 | {mgr.status.value} |")

        if mgr.sub_agents:
            lines.append(f"| 配下エージェント | {', '.join(f'{k} ({a.role})' for k, a in mgr.sub_agents.items())} |")
        lines.append("")

    # 足軽
    agent_specs = {
        "RDA": ("RequirementsDefinitionAgent", "agents/requirements_agent.py",
                "自然言語 → MLProjectSpec の構造化変換",
                "自然言語テキスト, UserRequirement リスト",
                "MLProjectSpec"),
        "SMA": ("ScrapingMonitoringAgent", "agents/scraping_agent.py",
                "netkeiba.com からのデータ取得・ファイル監視",
                "MLProjectSpec",
                "ScrapingResult (CSV パス群)"),
        "DOA": ("DataOrganizationAgent", "agents/data_organization_agent.py",
                "CSV/JSON データの結合・分割・クレンジング",
                "MLProjectSpec, ScrapingResult",
                "OrganizedData (training_data, prediction_entries, prediction_races)"),
        "FEA": ("FeatureEngineeringAgent", "agents/feature_engineering_agent.py",
                "特徴量生成・Label Encoding・派生特徴量・特徴量選択",
                "MLProjectSpec, OrganizedData",
                "FeatureSet (X_train, X_val, y_train, y_val, X_predict, predict_meta)"),
        "MTEA": ("ModelTrainingAgent", "agents/model_training_agent.py",
                 "LightGBM + Optuna チューニング・評価・MLflow 記録",
                 "MLProjectSpec, FeatureSet",
                 "TrainedModel (model, evaluation, best_params, feature_importance, mlflow_run_id)"),
        "POA": ("PredictionOutputAgent", "agents/prediction_agent.py",
                "学習済みモデルによる予測実行・結果構造化・JSON 出力",
                "TrainedModel, FeatureSet, upcoming_races",
                "PredictionResult (races, model_evaluation, feature_importance)"),
    }

    lines.append("### 3.5 足軽エージェント一覧")
    lines.append("")
    lines.append("| ID | クラス | ファイル | 責務 | 入力 | 出力 |")
    lines.append("|-----|--------|---------|------|------|------|")
    for key, (cls, fpath, desc, inp, out) in agent_specs.items():
        lines.append(f"| {key} | `{cls}` | `{fpath}` | {desc} | {inp} | {out} |")
    lines.append("")

    return "\n".join(lines)


def _intent_routing() -> str:
    return """
## 4. 意図分類とルーティング
...
def _communication_patterns() -> str:
    return """
## 6. 通信パターン
...
def _data_architecture(base_dir: str) -> str:
    lines = [
        "", "## 7. データアーキテクチャ", "",
        "```",
        "data/",
...
MLflow サーバーに接続できない場合、ローカルファイルストア (`mlflow/runs/`) に自動フォールバック。
接続テストは 3秒タイムアウトで判定。
...
def _prediction_output_format() -> str:
    return """
## 11. 予測結果出力フォーマット
...
| ボリューム | マウント先 | 内容 |
|-----------|-----------|------|
| `mlflow_data`（名前付き Docker volume） | `/mlflow` | MLflow DB + アーティファクト |
...
def update_spec(orch: Orchestrator, base_dir: str = "."):
    """ARCHITECTURE.md を生成してファイルに書き出す。"""
    content = generate_spec(orch, base_dir)
    spec_path = os.path.join(base_dir, "ARCHITECTURE.md")
