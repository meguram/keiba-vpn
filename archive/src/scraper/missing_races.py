...
def race_meeting_date_from_race_id(race_id: str) -> date | None:
    """race_id 先頭 8 桁を ``%Y%m%d`` として解釈できる場合に ``date`` を返す。

    **注意: netkeiba の ``race_id`` 先頭 8 桁は公式の「開催日」とは限らない。**
    開催日の判定・期間集計には ``race_lists`` のキー、ジョブの ``date``、
    または API 側の ``race_id``→開催日逆引き（``_resolve_date_for_race``）を使うこと。
    """
    s = str(race_id).strip()
